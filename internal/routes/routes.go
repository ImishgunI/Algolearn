...
func SetRoutes(r *gin.Engine, db *database.Database) {
	...
	hf := controller.NewFavoriteHandler(db)
	...
	r.POST("/favorites/toggle", hf.CreateFavorite)
	r.POST("/lessons/complete/", hl.SetDone)
	...
	r.GET("/favorites/", hf.GetFavorites)
	r.GET("/user/profile/", h.GetDataController)
	r.GET("/user/progress/", h.GetProgress)
