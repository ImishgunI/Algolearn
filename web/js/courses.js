...
function getVisualizationFunction(lessonTitle) {
    ...
    return visMap[lessonTitle] || (() => {
        d3.select("#vis").html("<p>Визуализация для этого урока пока не доступна</p>");
    });
}
...
function visualizeQuick(mode) {
    ...
    if (mode === "auto") {
        ...
        document.querySelector(".controls").innerHTML +=
            `<button onclick="stepQuick()">Далее</button>`;
...
function visualizeLinear(mode) {
    ...
    if (mode === "auto") {
        ...
        update();
        document.querySelector(".controls").innerHTML +=
            `<button onclick="stepLinear()">Далее</button>`;
...
function visualizeBinary(mode) {
    ...
    if (mode === "auto") {
        ...
        update();
        document.querySelector(".controls").innerHTML +=
            `<button onclick="stepBinary()">Далее</button>`;
...
function visualizeBFS(mode) {
    ...
    if (mode === "auto") {
        ...
        step();
        document.querySelector(".controls").innerHTML +=
            `<button onclick="stepBFS()">Далее</button>`;
        window.stepBFS = step;
    }
}
...
function visualizeMerge(mode) {
    ...
    const update = () => {
        ...
        step.arr.forEach((value, index) => {
            ...
            svg.append("rect")
                ...
                .attr("width", barWidth)
                .attr("height", height)
                .attr("fill", step.action === 'merge' ? "#4ecdc4" : 
                             step.action === 'split' ? "#ff6b6b" : "#0078D7");
    ...
    function getActionText(action) {
        switch(action) {
            case 'split': return 'Разделение';
            case 'merge': return 'Слияние';
            case 'base': return 'Базовый случай';
            default: return '';
        }
    }
    ...
    if (mode === "auto") {
        ...
        update();
        document.querySelector(".controls").innerHTML +=
            `<button onclick="stepMerge()">Далее</button>`;
...
function visualizeRecursion(type, mode) {
    ...
    const update = () => {
        ...
        svg.append("text")
            .attr("x", 400)
            .attr("y", currentY + 30)
            .text(`${step.type === 'call' ? 'Вызов' : 'Возврат'} ${step.function}(${step.argument})${step.result ? ` = ${step.result}` : ''}`)
            .attr("text-anchor", "middle")
            .attr("fill", "white")
            .attr("font-weight", "bold");
    ...
    if (mode === "auto") {
        ...
        update();
        document.querySelector(".controls").innerHTML +=
            `<button onclick="stepRecursion()">Далее</button>`;
...
function visualizeDPFib(mode) {
    ...
    for (let i = 2; i <= n; i++) {
        steps.push({
            index: i,
            dp: [...dp],
            calculation: `dp[${i}] = dp[${i-1}] + dp[${i-2}] = ${dp[i-1]} + ${dp[i-2]} = ${dp[i-1] + dp[i-2]}`,
    ...
    const update = () => {
        ...
        step.dp.forEach((value, index) => {
            ...
            svg.append("rect")
                ...
                .attr("fill", isCurrent ? "#ff6b6b" : 
                             isUsed ? "#4ecdc4" : "#0078D7")
                .attr("stroke", "black");
        ...
        if (step.index >= 2) {
            ...
            svg.append("polygon")
                .attr("points", `${arrowEndX},${startY + cellSize + 40} ${arrowEndX - 5},${startY + cellSize + 35} ${arrowEndX + 5},${startY + cellSize + 35}`)
                .attr("fill", "#4ecdc4");
    ...
    if (mode === "auto") {
        ...
        update();
        document.querySelector(".controls").innerHTML +=
            `<button onclick="stepDPFib()">Далее</button>`;
